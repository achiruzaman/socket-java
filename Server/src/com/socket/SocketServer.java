...
public class SocketServer implements Runnable {
    ...
    public SocketServer(ServerFrame frame){
       ...
       System.out.println("Eksekusi Tombol Start Server");
        clients = new ServerThread[50];
        ui = frame;
        db = new Database(ui.filePath);
	try{  
            port=Integer.parseInt(ui.txt_port_server.getText()); 
	    server = new ServerSocket(port);
            //port = server.getLocalPort();
            System.out.println("Portnya : "+port);
            System.out.println("Servernya : "+server);
            ui.txt_konten.append("Server dijalankan. IP : " + InetAddress.getLocalHost() + ", Port : " + port);
	    start(); 
            
        }
	catch(IOException ioe){  
            ui.txt_konten.append("Tidak dapat di-bind pada Port: " + port + "\nRetrying"); 
            ui.RetryStart(port);
	}
    }
	...
	try{  
            System.out.println(ui.txt_konten);
	    server = new ServerSocket(port);
            port = server.getLocalPort();
	    ui.txt_konten.append("Server kembali dijalankan. IP : " + InetAddress.getLocalHost() + ", Port : " + port);
	    start(); 
        }
	catch(IOException ioe){  
            ui.txt_konten.append("\nTidak dapat di-rebind pada Port: " + port + ": " + ioe.getMessage()); 
	}
    }
